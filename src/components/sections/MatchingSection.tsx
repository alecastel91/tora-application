    ...
    return (
        <section id="features" className="py-32 px-8 max-w-7xl mx-auto">
            <div className="mb-20">
                ...
                <div className="w-12 h-px bg-infrared mt-4" />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {steps.map((step, index) => (
                    <GlassCard
                        key={index}
                        className="group hover:bg-infrared/[0.02]"
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: index * 0.1 }}
                    >
                        <span className="text-4xl font-black text-white/5 mb-6 block group-hover:text-infrared/20 transition-colors">
                            {step.number}
                        </span>
